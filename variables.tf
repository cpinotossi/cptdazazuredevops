...
variable "target_subscription_id" {
  description = "List of target Subscription IDs (used for Connectivity, Identity, and Management). Can specify one or multiple subscriptions."
  type        = list(string)
  validation {
    condition     = length(var.target_subscription_id) > 0 && alltrue([for id in var.target_subscription_id : can(regex("^[0-9a-fA-F-]{36}$", id))])
    error_message = "At least one target subscription ID must be provided and all IDs must be valid GUIDs"
  }
}
